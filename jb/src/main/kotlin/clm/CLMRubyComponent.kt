...
class RubySymbolResolver : SymbolResolver {
    ...
    override fun clmElements(psiFile: PsiFile, clmExperimentResult: ClmExperimentResult?): List<ClmExperimentElements> {
        ...
        if (clmExperimentResult == null) return listOf()

        val infoBySymbol = mutableMapOf<PsiElement, MutableList<String>>()

        for (codeLevelMetric in clmExperimentResult.codeLevelMetrics) {
            val elements = listOf(
        ...
        val clmElements: List<ClmExperimentElements> = infoBySymbol.map {
            ClmExperimentElements(
                it.key.range,
