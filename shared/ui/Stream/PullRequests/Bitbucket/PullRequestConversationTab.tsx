...
export const PullRequestConversationTab = (props: {
	...
	const defaultMergeMethod = bbRepo.viewerDefaultMergeMethod || derivedState.defaultMergeMethod;
	...
	const fetchAvailableReviewers = async (e?) => {
		...
		const reviewers = await dispatch(
			api({
				method: "getReviewers",
				params: {
					owner: bbRepo.repoOwner,
					repo: bbRepo.repoName,
				},
			})
		).unwrap();
	...
	const requiredApprovingReviewCount = useMemo(() => {
		if (bbRepo.branchProtectionRules) {
			const rules = bbRepo.branchProtectionRules.nodes.find(
				rule =>
...
const Merge = (props: {
	...
	const { bbRepo, onSelect, action, defaultMergeMethod, mergeText } = props;
	return (
		<div style={{ padding: "5px 0" }}>
			{mergeText}
					...
					items={[
						{
							...
							disabled: !bbRepo.mergeCommitAllowed,
						...
						},
						{
							...
							disabled: !bbRepo.squashMergeAllowed,
						...
						},
						{
							...
							disabled: !bbRepo.rebaseMergeAllowed,
